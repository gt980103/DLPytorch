...
def jsma_attack(image, target, theta, gamma, model):
    ...
    print('测试样本扰动前的预测值：{}'.format(classes[predicted[0]]))
    ...
    while (iter < max_iters) and (current != target) and (search_domain.sum() != 0):
        ...
        output = model(var_sample)
        current = torch.max(output.data, 1)[1].item()
        print("epoch:{} p1:{} p2:{} label:{}".format(iter, p1, p2, classes[current]))
        iter += 1
...
if __name__=="__main__":
    ...
    # 模型对对抗性样本的预测结果
    outputs = model(torch.from_numpy(adv_image).requires_grad_().to(device))
    predicted = torch.max(outputs.data, 1)[1]
    print('测试样本扰动后的预测值：{}'.format(classes[predicted[0]]))
    ...
    adv_im = adv_image[0].transpose(1,2,0)
    adv_im = adv_im*std + mean
